...
def write(txt:str):
    keyboard.send_keys(keys = txt, with_spaces = True, with_newlines = True, pause = 0.02)
...
# Important !! Sets INTERNAL_COPY to true before copying to clipboard to bypass clipboard changed signal action.
def copy_it(txt:str):
...
def find_if_exists(png:str, area:tuple[int,int,int,int]):
    xy = pyautogui.locateCenterOnScreen(image = png, region = area, grayscale = True, confidence = 0.8)
    if xy == None: return False
    else: return True

def find_and_click(png:str, area:tuple[int,int,int,int], rt_click:bool = False, clicks:int = 1):
    if rt_click: button = 'Right'
    else: button = 'Left'
    xy = pyautogui.locateCenterOnScreen(image = png, region = area, grayscale = True, confidence = 0.8)
    if xy == None:
