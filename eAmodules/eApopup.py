...
def warning(text: str, yes_no: bool = False):
    ...
    warnbox.setText(
        "<span style='font-family:Lucida Sans;font-weight:600;font-style:italic;font-size:18pt;color:#aa4252;'>\
        WARNING !!!\
        </span>"
    )
    ...
    if yes_no:
        ...
        warnbox.addButton('OK', QMessageBox.YesRole)
    ok = warnbox.exec()
    return not ok
...
def notify(text: str):
    ...
    notibox.setText(
        "<span style='font-family:Lucida Sans;font-weight:600;font-style:italic;font-size:18pt;color:#3b4252;'>\
        Notifying !!!\
        </span>"
    )
...
def timed_confirm(text: str, msec: int):
    ...
    confbox.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint)
    confbox.setText(
        f"<span style='font-family:Lucida Sans;font-weight:600;font-style:italic;font-size:18pt;color:#3b4252;'>\
        Waiting..for {count_down} seconds\
        </span>"
    )
    confbox.setStyleSheet(eAstyles.CONFBOX)
    confbox.setInformativeText(text)
...
def get_text(text: str, default_text: str = "", password: bool = False):
    ...
    if password:
        inputbox.setTextEchoMode(QLineEdit.Password)
