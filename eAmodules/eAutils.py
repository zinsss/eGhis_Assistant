...
import main
from eAmodules import eAinput, eApopup, eAcalendar
...
def jmno_to_birthd(jmno: str):
    ...
    if int(jmno[7]) in year19s:
        return "19" + jmno[:6]
    elif int(jmno[7]) in year20s:
        return "20" + jmno[:6]
    else:
        return
...
def jmno_isValid(jmno_with_dash: str):
    # '-'가 있는 주민번호만 받기로 하고
    if "-" not in jmno_with_dash:
        return False
    # 7번째 (파이썬에서는 0부터 시작이니 6) 자리에 '-'가 있어야하고
    elif jmno_with_dash[6] != "-":
        return False
    # '-' 포함 총 14자리여야 하고
    elif len(jmno_with_dash) != 14:
        return False
    # '-'를 제외 하면 모두 숫자로 구성되어야 하고
    elif jmno_with_dash.replace("-", "").isdigit() is False:
        return False
    # 앞자리는 날짜양식에 맞아야 하며
    elif QDate.fromString(jmno_with_dash[:6], "yyMMdd").isValid() is False:
        return False
    # 뒷자리 첫번째 숫자는 0이나 9가 아니어야 진행 가능.(생존자 없음 1800년대)
    elif int(jmno_with_dash[7]) not in [1, 2, 3, 4, 5, 6, 7, 8]:
        return False
    else:
        return True
...
def check_underaged(self, jmno_with_dash: str):
    ...
    if int(birthday_yyyyMMdd[:4]) > target_birth_year:
        ...
        birthday_this_year = QDate.fromString(
            str(self.infos.date_today.year()) + birthday_yyyyMMdd[4:], "yyyyMMdd")
...
def lwg_copy_or_paste(self, listwdg: str, copy: bool = False):
    '''
...
def lwg_add_single_line_item(self, listwdg: str, popup_text: str = ""):
    lwg = getattr(self, listwdg)
    ok = eApopup.confirm(text='새로운 항목을 추가할까요?')
    if not ok:
        return
    else:
...
def send_discord(self, text: str):
    ...
    if not self.stgn_messenger_btn.isChecked():
        return
    # 메신저 설정에 서버(WEBHOOK)가 있는지 확인하고.
    # TODO 공란이라고 무조건 보내지는 건 아니니까... confirmation 추가하는게..
    if self.stgn_messenger_server_pte.toPlainText() == "":
        return
    # 실제로 메세지 보내기
...
def daily_report_discord(self):
    ...
    for i in range(self.reminders_lwg.count()):
        reminders_active = reminders_active + \
            f'{self.reminders_lwg.item(i).text()}\n'
