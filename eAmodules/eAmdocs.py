...
def connect_DB():
    ...
    con.execute('CREATE TABLE if not exists MedDocs(Title TEXT UNIQUE, Contents TEXT)')    
...
def order_change(self):
    ...
    cur = con.cursor()
    # DB내 기존 목록을 모두 지우고..
    con.execute(f'DELETE FROM MedDocs')
    for item in items:
...
def add_new_doc(self):
    ok = eApopup.confirm_big(text = f"새로운 문서를 추가할까요?")
    if not ok: return
    ...
    cur = con.cursor()
    # 편집항목 DB내 UPDATE
    con.execute(f'INSERT INTO MedDocs VALUES (?, ?);', new_doc)
    # DB 저장 후 연결 해제.
    con.commit()
    con.close()
...
def delete_doc(self):
    ...
    cur = con.cursor()
    # 편집항목 DB내 UPDATE
    con.execute(f'DELETE FROM MedDocs WHERE Title = "{title}"')
    # DB 저장 후 연결 해제.
    con.commit()
    con.close()
...
def edit_mode_toggler(self, toggle:bool):
    ...
    if toggle == False:
        load_contents(self)
    else: self.mdoc_contents_pte.setFocus()
...
def edit_btn_clicked(self):
    ...
    if edit_mode:
        ...
        ok = eApopup.warning(text = f"저장하지 않고 편집을 취소할까요?")
        if not ok:
            self.mdoc_edit_btn.setChecked(True)
            return
...
def edit_doc(self):
    ...
    cur = con.cursor()
    # 편집항목 DB내 UPDATE
    con.execute(f"UPDATE MedDocs SET Title = '{title_new}', Contents = '{contents_new}' WHERE Title = '{target_title}'")
    # DB 저장 후 연결 해제.
    con.commit()
    con.close()
