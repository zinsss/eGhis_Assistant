...
import main
from eAmodules import eAwinauto, eAinput, eAutils, eAxys, eAstr, eApopup, eAlabeler, eAcovid19R, eAvacsys
...
def physical_tx(self):
    ...
    if choice == 'CLOSE': return
    elif choice == menu_items[1]: eAinput.rx_input('물치')
...
# TODO needs working.
def other_comments():
    menu_items = ['보존적 치료', '감기 기본', '복불 기본', '엑스레이 정상', '골밀도 FORM', '연골주사 FORM']
    choice = eApopup.menu(title = "기타 코멘트 모음", buttons = menu_items)
    if choice == 'CLOSE': return
    elif choice == menu_items[0]: eAinput.sx_cnp('보존;')
    elif choice == menu_items[1]: eAinput.sx_cnp('pharyn;')
    elif choice == menu_items[2]: eAinput.sx_cnp('abd;')
    elif choice == menu_items[3]: eAinput.sx_cnp('엑스레이;')
    elif choice == menu_items[4]: eAinput.sx_cnp('골밀도;')
    elif choice == menu_items[5]: eAinput.sx_cnp('연골;')
...
def vac_sys_manager(self):
    ...
    if choice == 'CLOSE': return
    elif choice == options[0]: vac_sys_input(self)
    elif choice == options[1]: vac_sys_start(self)
    elif choice == options[2]: eAwinauto.vaccine_cont_all(self)
...
def vac_sys_input(self):
    ...
    if choice == 'CLOSE': return
        ...
        try: eAwinauto.vaccine_pt(self, vac_sys.index(choice), currentPT['ptjmno'])
        except: eApopup.notify(text = "다시 확인해보세요!!")
...
def covid_19_menu():
    ...
    if choice == 'CLOSE': return
    elif choice == menu_items[0]: RAT()
    elif choice == menu_items[1]: RAT_results()
...
def RAT_results():
    ...
    if choice == 'CLOSE': return
    elif choice == menu_items[0]: RAT_result_input()
    else: 
...
def RAT_result_input(positive:bool = False):
    ...
    eAinput.find_and_click(png = 'resources/macros/rat_rx_los.png', area = (160, 1000, 1100, 1100), rt_click = True)
...
#= 키트 처방여부 확인
def flu_kit_ordered():
    ordered = eAinput.find_if_exists(png = 'resources/macros/flu_kit_rx_los.png', area = (160, 1000, 1100, 1100))
    if not ordered:
        ok = eApopup.confirm(text = "플루 키트가 처방되지 않았습니다.\n타기관 결과? 금일 이전 결과?\n금일 처방없이 진행할까요?")
        if ok:
            return
        else:
            influenza_candidate()
...
def influenza_result():
    ...
    if nega_posi == 'CLOSE': return 'CLOSE'
        ...
        if not order: return
        else: influenza_tamiflu(flu_type)
...
def influenza(self):
    ...
    if choice == 'CLOSE': return
    elif choice == menu_items[0]: influenza_candidate()
